...
*/
    ...
    public any function getRCValue( string propName, any defaultValue = '' ) {
        if ( structKeyExists( request, 'context' ) &&
             structKeyExists( request.context, arguments.propName ) ) {
            return request.context[ arguments.propName ];
        } else {
	...
	public string function getSubsystem( string action = request.action ) {
		if ( actionSpecifiesSubsystem( arguments.action ) ) {
			return listFirst( arguments.action, variables.framework.subsystemDelimiter );
		}
	...
	public void function onError( any exception, string event ) {
		try {
			...
			internalFrameworkTrace( 'onError( #arguments.exception.message#, #arguments.event# ) called' );
			setupRequestWrapper( false );
			onRequest( '' );
            frameworkTraceRender();
		} catch ( any e ) {
			failure( e, 'onError' );
			failure( arguments.exception, arguments.event, true );
            frameworkTraceRender();
		}
	...
	public any function onRequestStart( string targetPath ) {
		...
		if ( listFindNoCase( variables.framework.unhandledExtensions, listLast( arguments.targetPath, '.' ) ) ||
				REFindNoCase( '^(' & variables.framework.unhandledPathRegex & ')', arguments.targetPath ) ) {
    ...
    private void function deprecated( boolean throwit, string message ) {
        if ( arguments.throwit ) {
            ...
            out.println( "FW/1: DEPRECATED: " & arguments.message );
        }
    }
	...
	private void function doController( struct tuple, string method, string lifecycle ) {
		...
		if ( structKeyExists( cfc, arguments.method ) ) {
			...
			try {
				...
				evaluate( 'cfc.#method#( rc = request.context, method = arguments.lifecycle )' );
			} catch ( any e ) {
				setCfcMethodFailureInfo( cfc, arguments.method );
				rethrow;
			}
	...
	private void function failure( any exception, string event, boolean indirect = false, boolean early = false ) {
		...
		if ( arguments.early ) {
			...
			writeOutput( '<h#h#>' & ( arguments.indirect ? 'Original exception ' : 'Exception' ) & ' in #arguments.event#</h#h#>' );
			if ( structKeyExists( request, 'failedAction' ) ) {
				writeOutput( '<p>The action #request.failedAction# failed.</p>' );
			}
			writeOutput( '<h#1+h#>#arguments.exception.message#</h#1+h#>' );
		}

		writeOutput( '<p>#arguments.exception.detail# (#arguments.exception.type#)</p>' );
		dumpException(arguments.exception);
	...
	private any function getCachedComponent( string type, string subsystem, string section ) {
		...
		var beanName = arguments.section & arguments.type;

		if ( !structKeyExists( cache[ types ], componentKey ) ) {
			lock name="fw1_#application.applicationName#_#variables.framework.applicationKey#_#arguments.type#_#componentKey#" type="exclusive" timeout="30" {
				if ( !structKeyExists( cache[ types ], componentKey ) ) {
					if ( usingSubsystems() && hasSubsystemBeanFactory( arguments.subsystem ) && getSubsystemBeanFactory( arguments.subsystem ).containsBean( beanName ) ) {
						...
						if ( arguments.type == 'controller' && arguments.section == variables.magicApplicationController ) {
							// treat this (Application.cfc) as a controller:
							cfc = this;
						} else if ( cachedFileExists( cfcFilePath( request.cfcbase ) & subsystemDir & types & '/' & arguments.section & '.cfc' ) ) {
							// we call createObject() rather than new so we can control initialization:
							if ( request.cfcbase == '' ) {
								cfc = createObject( 'component', subsystemDot & types & '.' & arguments.section );
							} else {
								cfc = createObject( 'component', request.cfcbase & '.' & subsystemDot & types & '.' & arguments.section );
							}
					...
					if ( isObject( cfc ) ) {
                        if ( arguments.type == 'controller' ) injectFramework( cfc );
	...
	private any function getController( string section, string subsystem = getDefaultSubsystem() ) {
		var _controller_fw1 = getCachedComponent( 'controller', arguments.subsystem, arguments.section );
	...
	private string function parseViewOrLayoutPath( string path, string type ) {
		...
		return customizeViewOrLayoutPath( pathInfo, arguments.type, '#pathInfo.base##arguments.type#s/#pathInfo.path#.cfm' );
	...
	private struct function processRoutes( string path, array routes = getRoutes() ) {
		for ( var routePack in arguments.routes ) {
			for ( var route in routePack ) {
				...
				if ( route == '$RESOURCES' ) {
					var routeMatch = processRoutes( arguments.path, getResourceRoutes( routePack[ route ] ) );
				} else {
					var routeMatch = processRouteMatch( route, routePack[ route ], arguments.path );
				}
